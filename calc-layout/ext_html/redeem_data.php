<h4 class="main-calc__title _small label-text">
    <span class="main-calc__title_icon"></span>
    <span class="main-calc__title_text label-text">
        <span>Данные для выкупа</span>
        <span class="circle-info">?</span>
    </span>
    <span class="calc-tooltip">
        <span class="calc-tooltip__title">ДАННЫЕ ДЛЯ ВЫКУПА</span>
        <span class="calc-tooltip__text">
            Данные для выкупа нужны для наших менеджеров, чтобы сделать детальный просчет.
            Впишите данные в ручную или прикрепите Excel-файл.
        </span>
    </span>

</h4>

<div class="main-calc-bordered order-data-dimensions redeem-data-one" data-container-redeem="1">
    <div class="close-cross">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
             width="29.5558703725" height="29.5558703725"
             viewBox="154.70573108694998 97.92453478204999 29.5558703725 29.5558703725" xml:space="preserve">
            <g transform="matrix(1 0 0 1 169.4836662732 112.7024699683)" id="k3KNRoqNc6poTgMk0-mS0">
                <g style="">
                    <g transform="matrix(0.5361434886 0 0 0.5361434886 0 0)" id="TtVKn0Vpwc73qYTht60LM">
                        <path class="cross-border"
                              style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                              transform=" translate(0, 0)"
                              d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"
                              stroke-linecap="round"/>
                    </g>
                    <g transform="matrix(0.4501312202 0 0 0.4501312202 0 0)" id="UNUcbtI9Ts4pakmCW0QUX">
                        <path class="cross-back"
                              style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                              transform=" translate(0, 0)"
                              d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"
                              stroke-linecap="round"/>
                    </g>
                    <g transform="matrix(0.4487730328 0 0 0.4487730328 0 0)" id="_16PNKPtF4U3-sJUah521">
                        <path class="cross-path"
                              style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                              transform=" translate(0, 0)"
                              d="M 14.14294 -14.1405 C 12.58094 -15.702499999999999 10.045939999999998 -15.702499999999999 8.484939999999998 -14.1405 L -0.00006000000000128125 -5.656499999999999 L -8.484060000000001 -14.1395 C -10.04606 -15.7015 -12.580060000000001 -15.7015 -14.14206 -14.1395 C -15.70406 -12.5775 -15.70406 -10.0435 -14.14206 -8.4835 L -5.657060000000001 0.0005000000000006111 L -14.14206 8.4845 C -15.70406 10.0465 -15.70406 12.5805 -14.14206 14.1405 C -12.580060000000001 15.702499999999999 -10.04606 15.702499999999999 -8.48406 14.1405 L -0.000059999999999504894 5.657499999999999 L 8.48394 14.1405 C 10.04594 15.702499999999999 12.57994 15.702499999999999 14.141940000000002 14.1405 C 15.703940000000001 12.5785 15.703940000000001 10.043499999999998 14.141940000000002 8.4845 L 5.656940000000002 0.0005000000000006111 L 14.141940000000002 -8.4835 C 15.703940000000001 -10.0465 15.703940000000001 -12.5795 14.142940000000001 -14.1405 z"
                              stroke-linecap="round"/>
                    </g>
                </g>
            </g>
        </svg>
    </div>

    <div class="main-calc-buy-info redeem-content">

        <div class="main-calc-upload redeem-square-outer">
            <input type="file" class="main-calc-upload__input photo-input" style="display: none;">
            <div class="main-calc-upload__icon redeem-upload-icon"></div>
            <div class="main-calc-upload__plus redeem-inner-square redeem-inner-square-plus">
                <div class="redeem-data-plus">+</div>
            </div>
            <div class="main-calc-upload__minus redeem-inner-square redeem-inner-square-minus">
                <div class="redeem-data-minus">-</div>
            </div>
            <img class="main-calc-upload__item selected-photo" src="" alt="Выбранное фото">
        </div>

        <div class="main-calc-buy-info__form">

            <div class="group-input">
                <div class="group-input__title">Наименование товара</div>
                <div class="group-input__input">
                    <input class="redeem-input redeem-name-input" type="text" required id="redeem-name-input">
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Цена за единицу</div>
                <div class="group-input__input">
                    <input class="js-validate-num redeem-currency-input" type="number" id="redeem-price">
                    <div class="dropdown">
                        <div class="dropdown-toggle currency-toggle" data-currency="CNY">
                            ¥
                            <svg class="dropdown-list-currency-arrow" xmlns="http://www.w3.org/2000/svg"
                                 xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="32.38141722065728"
                                 height="22.59489123485507"
                                 viewBox="110.22394056737565 250.64901537709036 32.38141722065728 22.59489123485507"
                                 xml:space="preserve">
                                            <desc>Created with Fabric.js 5.3.0</desc>
                                <g transform="matrix(0 0.979842546 0.979842546 0 126.4146491777 261.9464609945)"
                                   id="DLgAgrXuFMXOEYFpJSRZM">
                                    <path class="svg-arrow"
                                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                                          transform=" translate(0, 0)"
                                          d="M 8.536 -3.5215 L -3.183 -14.8535 C -3.9269999999999996 -15.5975 -4.931 -16.0135 -5.999 -16.0135 C -7.068 -16.0135 -8.072 -15.5975 -8.827 -14.8415 C -9.583 -14.0855 -9.999 -13.0815 -9.999 -12.0135 C -9.999 -10.945500000000001 -9.583 -9.9405 -8.827 -9.185500000000001 L -0.29100000000000037 -0.6915000000000013 C -0.10300000000000037 -0.5035000000000014 -3.885780586188048e-16 -0.25350000000000134 -3.885780586188048e-16 0.013499999999998624 C -3.885780586188048e-16 0.28049999999999864 -0.10400000000000038 0.5314999999999986 -0.29300000000000037 0.7204999999999986 L -8.824000000000002 9.1815 C -9.583000000000002 9.9415 -9.999000000000002 10.9465 -9.999000000000002 12.0135 C -9.999000000000002 13.0805 -9.583000000000002 14.0855 -8.827000000000002 14.8415 C -8.072000000000001 15.5975 -7.067000000000002 16.0135 -5.999000000000002 16.0135 C -4.931000000000003 16.0135 -3.9260000000000024 15.5975 -3.170000000000002 14.8415 L -1.564000000000002 13.2355 L 8.519999999999998 3.5645000000000007 C 9.478999999999997 2.6055000000000006 9.998999999999999 1.3495000000000008 9.998999999999999 0.013500000000000512 C 9.998999999999999 -1.3224999999999996 9.479 -2.5774999999999997 8.535999999999998 -3.5214999999999996 z"
                                          stroke-linecap="round"/>
                                </g>
                                        </svg>
                        </div>
                        <ul class="dropdown-list">
                            <li><span class="currency-sign" data-currency="USD">$</span></li>
                            <li><span class="currency-sign" data-currency="RU">₽</span></li>
                            <li><span class="currency-sign" data-currency="CNY">¥</span></li>
                        </ul>
                    </div>
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Кол-во товаров</div>
                <div class="group-input-increment">
                    <span class="group-input-increment__minus">-</span>
                    <input class="js-validate-num redeem-count" type="number" value="1" id="redeem-quantity" name="count" required>
                    <span class="group-input-increment__plus">+</span>
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Размер</div>
                <div class="group-input__input">
                    <input class="redeem-input redeem-size" type="text" id="redeem-size">
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Цвет</div>
                <div class="group-input__input">
                    <input class="redeem-input redeem-color" type="text" id="redeem-color">
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Ссылка интернет магазина на товар</div>
                <div class="group-input__input">
                    <input class="redeem-input redeem-url-input" type="text" id="redeem-url-input" placeholder="https://">
                    <div class="input-notice"></div>
                </div>
            </div>

            <div class="group-input">
                <div class="group-input__title">Примечание</div>
                <div class="group-input__input">
                    <input class="redeem-input redeem-note" type="text" id="redeem-url-input">
                    <div class="input-notice"></div>
                </div>
            </div>

        </div>
    </div>

<!--    <div class="redeem-line2 redeem-params-margin">-->
<!--    </div>-->

</div>

<div class="redeem-buttons" style="display: flex; width: 97%;">
    <div class="add-container redeem-add" style="width: 180px;">
        <div class="add-button">
            <span class="plus">+</span>
            <span class="add">Добавить товар</span>
        </div>
    </div>
    <button class="main-calc-button calc-type-button tnved-calc-button submit-redeem-data submit-excel-file">
        <span class="main-calc-button__upload_icon">
            <svg width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M46.9103 28.2876C44.6517 28.2876 42.8207 30.1187 42.8207 32.3773V42.8207H8.19493V32.3773C8.19493 30.1187 6.36386 28.2876 4.10528 28.2876C1.8467 28.2876 0.015625 30.1187 0.015625 32.3773V46.9104C0.015625 49.169 1.8467 51 4.10528 51H46.9103C49.1689 51 51 49.169 51 46.9104V32.3773C51 30.1187 49.1689 28.2876 46.9103 28.2876Z" fill="white"/>
                <path d="M16.853 18.5281L21.4182 13.963V29.2004C21.4182 31.459 23.2493 33.29 25.5078 33.29C27.7664 33.29 29.5975 31.459 29.5975 29.2004V13.9629L34.1626 18.528C34.9611 19.3266 36.0077 19.7259 37.0544 19.7259C38.1011 19.7259 39.1476 19.3266 39.9462 18.528C41.5434 16.9309 41.5434 14.3416 39.9462 12.7444L28.3996 1.19796C26.8026 -0.399321 24.213 -0.399321 22.616 1.19796L11.0694 12.7446C9.4723 14.3417 9.4723 16.931 11.0694 18.5281C12.6665 20.1253 15.256 20.1253 16.853 18.5281Z" fill="white"/>
            </svg>
        </span>
        <span>Прикрепить Excel-файл</span>
    </button>
    <button class="main-calc-button calc-type-button tnved-calc-button submit-redeem-data blank-excel-file">
        <span class="main-calc-button__upload_icon">
            <svg width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0_6504_12668)">
                <mask id="mask0_6504_12668" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="51" height="51">
                <path d="M51 0H0V51H51V0Z" fill="white"/>
                </mask>
                <g mask="url(#mask0_6504_12668)">
                <path d="M46.9103 28.2876C44.6517 28.2876 42.8207 30.1187 42.8207 32.3773V42.8207H8.19493V32.3773C8.19493 30.1187 6.36386 28.2876 4.10528 28.2876C1.8467 28.2876 0.015625 30.1187 0.015625 32.3773V46.9104C0.015625 49.169 1.8467 51 4.10528 51H46.9103C49.1689 51 51 49.169 51 46.9104V32.3773C51 30.1187 49.1689 28.2876 46.9103 28.2876Z" fill="white"/>
                <path d="M34.1622 14.7619L29.597 19.327V4.0896C29.597 1.831 27.7659 1.975e-07 25.5074 0C23.2488 -1.974e-07 21.4177 1.831 21.4177 4.0896V19.3271L16.8526 14.762C16.0541 13.9634 15.0075 13.5641 13.9608 13.5641C12.9141 13.5641 11.8676 13.9634 11.069 14.762C9.4718 16.3591 9.4718 18.9484 11.069 20.5456L22.6156 32.0921C24.2126 33.6894 26.8022 33.6894 28.3992 32.0921L39.9458 20.5454C41.5429 18.9483 41.5429 16.359 39.9458 14.7619C38.3487 13.1647 35.7592 13.1647 34.1622 14.7619Z" fill="white"/>
                </g>
                </g>
                <defs>
                <clipPath id="clip0_6504_12668">
                <rect width="51" height="51" fill="white"/>
                </clipPath>
                </defs>
            </svg>
        </span>
        <span>Образец бланка Excel</span>
    </button>
    <div class="selected-file-name">
        <div class="del-file">
            <div class="close-cross custom-span-tnved"
                 style="left: unset; margin-left: unset; margin-top: 2px; display: block;">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                     width="29.5558703725" height="29.5558703725"
                     viewBox="154.70573108694998 97.92453478204999 29.5558703725 29.5558703725" xml:space="preserve">
                    <g transform="matrix(1 0 0 1 169.4836662732 112.7024699683)" id="k3KNRoqNc6poTgMk0-mS0">
                        <g style="">
                            <g transform="matrix(0.5361434886 0 0 0.5361434886 0 0)" id="TtVKn0Vpwc73qYTht60LM">
                                <path class="cross-border"
                                      style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                                      transform=" translate(0, 0)"
                                      d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"
                                      stroke-linecap="round"/>
                            </g>
                            <g transform="matrix(0.4501312202 0 0 0.4501312202 0 0)" id="UNUcbtI9Ts4pakmCW0QUX">
                                <path class="cross-back"
                                      style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                                      transform=" translate(0, 0)"
                                      d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"
                                      stroke-linecap="round"/>
                            </g>
                            <g transform="matrix(0.4487730328 0 0 0.4487730328 0 0)" id="_16PNKPtF4U3-sJUah521">
                                <path class="cross-path"
                                      style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"
                                      transform=" translate(0, 0)"
                                      d="M 14.14294 -14.1405 C 12.58094 -15.702499999999999 10.045939999999998 -15.702499999999999 8.484939999999998 -14.1405 L -0.00006000000000128125 -5.656499999999999 L -8.484060000000001 -14.1395 C -10.04606 -15.7015 -12.580060000000001 -15.7015 -14.14206 -14.1395 C -15.70406 -12.5775 -15.70406 -10.0435 -14.14206 -8.4835 L -5.657060000000001 0.0005000000000006111 L -14.14206 8.4845 C -15.70406 10.0465 -15.70406 12.5805 -14.14206 14.1405 C -12.580060000000001 15.702499999999999 -10.04606 15.702499999999999 -8.48406 14.1405 L -0.000059999999999504894 5.657499999999999 L 8.48394 14.1405 C 10.04594 15.702499999999999 12.57994 15.702499999999999 14.141940000000002 14.1405 C 15.703940000000001 12.5785 15.703940000000001 10.043499999999998 14.141940000000002 8.4845 L 5.656940000000002 0.0005000000000006111 L 14.141940000000002 -8.4835 C 15.703940000000001 -10.0465 15.703940000000001 -12.5795 14.142940000000001 -14.1405 z"
                                      stroke-linecap="round"/>
                            </g>
                        </g>
                    </g>
                </svg>
            </div>
        </div>
    </div>
</div>

<!--<div class="order-data-dimensions redeem-data-one" data-container-redeem="1">-->
<!--    <div class="close-cross">-->
<!--        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"-->
<!--             width="29.5558703725" height="29.5558703725"-->
<!--             viewBox="154.70573108694998 97.92453478204999 29.5558703725 29.5558703725" xml:space="preserve">-->
<!--            <g transform="matrix(1 0 0 1 169.4836662732 112.7024699683)" id="k3KNRoqNc6poTgMk0-mS0">-->
<!--                <g style="">-->
<!--                    <g transform="matrix(0.5361434886 0 0 0.5361434886 0 0)" id="TtVKn0Vpwc73qYTht60LM">-->
<!--                        <path class="cross-border"-->
<!--                              style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                              transform=" translate(0, 0)"-->
<!--                              d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"-->
<!--                              stroke-linecap="round"/>-->
<!--                    </g>-->
<!--                    <g transform="matrix(0.4501312202 0 0 0.4501312202 0 0)" id="UNUcbtI9Ts4pakmCW0QUX">-->
<!--                        <path class="cross-back"-->
<!--                              style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                              transform=" translate(0, 0)"-->
<!--                              d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"-->
<!--                              stroke-linecap="round"/>-->
<!--                    </g>-->
<!--                    <g transform="matrix(0.4487730328 0 0 0.4487730328 0 0)" id="_16PNKPtF4U3-sJUah521">-->
<!--                        <path class="cross-path"-->
<!--                              style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                              transform=" translate(0, 0)"-->
<!--                              d="M 14.14294 -14.1405 C 12.58094 -15.702499999999999 10.045939999999998 -15.702499999999999 8.484939999999998 -14.1405 L -0.00006000000000128125 -5.656499999999999 L -8.484060000000001 -14.1395 C -10.04606 -15.7015 -12.580060000000001 -15.7015 -14.14206 -14.1395 C -15.70406 -12.5775 -15.70406 -10.0435 -14.14206 -8.4835 L -5.657060000000001 0.0005000000000006111 L -14.14206 8.4845 C -15.70406 10.0465 -15.70406 12.5805 -14.14206 14.1405 C -12.580060000000001 15.702499999999999 -10.04606 15.702499999999999 -8.48406 14.1405 L -0.000059999999999504894 5.657499999999999 L 8.48394 14.1405 C 10.04594 15.702499999999999 12.57994 15.702499999999999 14.141940000000002 14.1405 C 15.703940000000001 12.5785 15.703940000000001 10.043499999999998 14.141940000000002 8.4845 L 5.656940000000002 0.0005000000000006111 L 14.141940000000002 -8.4835 C 15.703940000000001 -10.0465 15.703940000000001 -12.5795 14.142940000000001 -14.1405 z"-->
<!--                              stroke-linecap="round"/>-->
<!--                    </g>-->
<!--                </g>-->
<!--            </g>-->
<!--        </svg>-->
<!--    </div>-->
<!--    <div class="redeem-content">-->
<!--        <div class="redeem-square-outer">-->
<!--            <input type="file" class="photo-input" style="display: none;">-->
<!--            <div class="redeem-upload-icon"></div>-->
<!--            <div class="redeem-inner-square redeem-inner-square-plus">-->
<!--                <div class="redeem-data-plus">+</div>-->
<!--            </div>-->
<!--            <div class="redeem-inner-square redeem-inner-square-minus">-->
<!--                <div class="redeem-data-minus">-</div>-->
<!--            </div>-->
<!--            <img class="selected-photo" src="" alt="Выбранное фото">-->
<!--        </div>-->
<!--        <div>-->
<!--            <div class="redeem-column">-->
<!--                <div class="redeem-name-container">-->
<!--                    <label for="redeem-name-input" class="redeem-name-input-label">Наименование товара</label>-->
<!--                    <input type="text" id="redeem-name-input" class="calc-text-input redeem-input redeem-name-input">-->
<!--                    <div class="input-notice"></div>-->
<!--                </div>-->
<!--                <div class="container-currency redeem-price-content">-->
<!--                    <label for="redeem-price" class="label-price redeem-price">Цена за единицу</label>-->
<!--                    <div style="display: flex">-->
<!--                        <input type="text" class="custom-input-currency redeem-currency redeem-currency-input"-->
<!--                               id="redeem-price">-->
<!--                        <div class="dropdown redeem-currency">-->
<!--                            <div class="dropdown-toggle" data-currency="CNY">-->
<!--                                ¥-->
<!--                                <svg class="dropdown-list-currency-arrow" xmlns="http://www.w3.org/2000/svg"-->
<!--                                     xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="32.38141722065728"-->
<!--                                     height="22.59489123485507"-->
<!--                                     viewBox="110.22394056737565 250.64901537709036 32.38141722065728 22.59489123485507"-->
<!--                                     xml:space="preserve">-->
<!--                                    <g transform="matrix(0 0.979842546 0.979842546 0 126.4146491777 261.9464609945)"-->
<!--                                       id="DLgAgrXuFMXOEYFpJSRZM">-->
<!--                                        <path class="svg-arrow"-->
<!--                                              style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                                              transform=" translate(0, 0)"-->
<!--                                              d="M 8.536 -3.5215 L -3.183 -14.8535 C -3.9269999999999996 -15.5975 -4.931 -16.0135 -5.999 -16.0135 C -7.068 -16.0135 -8.072 -15.5975 -8.827 -14.8415 C -9.583 -14.0855 -9.999 -13.0815 -9.999 -12.0135 C -9.999 -10.945500000000001 -9.583 -9.9405 -8.827 -9.185500000000001 L -0.29100000000000037 -0.6915000000000013 C -0.10300000000000037 -0.5035000000000014 -3.885780586188048e-16 -0.25350000000000134 -3.885780586188048e-16 0.013499999999998624 C -3.885780586188048e-16 0.28049999999999864 -0.10400000000000038 0.5314999999999986 -0.29300000000000037 0.7204999999999986 L -8.824000000000002 9.1815 C -9.583000000000002 9.9415 -9.999000000000002 10.9465 -9.999000000000002 12.0135 C -9.999000000000002 13.0805 -9.583000000000002 14.0855 -8.827000000000002 14.8415 C -8.072000000000001 15.5975 -7.067000000000002 16.0135 -5.999000000000002 16.0135 C -4.931000000000003 16.0135 -3.9260000000000024 15.5975 -3.170000000000002 14.8415 L -1.564000000000002 13.2355 L 8.519999999999998 3.5645000000000007 C 9.478999999999997 2.6055000000000006 9.998999999999999 1.3495000000000008 9.998999999999999 0.013500000000000512 C 9.998999999999999 -1.3224999999999996 9.479 -2.5774999999999997 8.535999999999998 -3.5214999999999996 z"-->
<!--                                              stroke-linecap="round"/>-->
<!--                                    </g>-->
<!--                                </svg>-->
<!--                            </div>-->
<!--                            <ul class="dropdown-list">-->
<!--                                <li><span class="currency-sign" data-currency="USD">$</span></li>-->
<!--                                <li><span class="currency-sign" data-currency="RU">₽</span></li>-->
<!--                                <li><span class="currency-sign" data-currency="CNY">¥</span></li>-->
<!--                            </ul>-->
<!--                        </div>-->
<!--                        <div class="input-notice"></div>-->
<!--                    </div>-->
<!--                </div>-->
<!--                <div class="custom-input-container-dimensions redeem-quantity">-->
<!--                    <label for="redeem-quantity" class="custom-input-label-dimensions">Кол-во товаров</label>-->
<!--                    <div class="minus-plus-dimensions">-->
<!--                        <span class="custom-input-addon custom-input-left-addon-dimensions"-->
<!--                              id="redeem-decrease">-</span>-->
<!--                        <input type="text" id="redeem-quantity" class="custom-input-field-dimensions redeem-count"-->
<!--                               value="1"/>-->
<!--                        <span class="custom-input-addon custom-input-right-addon-dimensions"-->
<!--                              id="redeem-increase">+</span>-->
<!--                        <div class="input-notice"></div>-->
<!--                    </div>-->
<!--                </div>-->
<!--                <div class="redeem-name-container redeem-params-container">-->
<!--                    <label for="redeem-name-input" class="redeem-name-input-label">Размер</label>-->
<!--                    <input type="text" id="redeem-size"-->
<!--                           class="calc-text-input redeem-params-input redeem-name-input redeem-size">-->
<!--                    <div class="input-notice"></div>-->
<!--                </div>-->
<!--                <div class="redeem-name-container redeem-params-container">-->
<!--                    <label for="redeem-name-input" class="redeem-name-input-label">Цвет</label>-->
<!--                    <input type="text" id="redeem-color"-->
<!--                           class="calc-text-input redeem-params-input redeem-name-input redeem-color">-->
<!--                    <div class="input-notice"></div>-->
<!--                </div>-->
<!--            </div>-->
<!--            <div class="redeem-line2">-->
<!--                <div class="redeem-name-container" style="width: 100%;">-->
<!--                    <label for="redeem-url-input" class="redeem-name-input-label">Ссылка интернет магазина на-->
<!--                        товар</label>-->
<!--                    <input type="text" id="redeem-url-input" class="calc-text-input redeem-input redeem-url-input"-->
<!--                           placeholder="https://">-->
<!--                    <div class="input-notice"></div>-->
<!--                </div>-->
<!--                <div class="redeem-name-container" style="width: 100%;">-->
<!--                    <label for="redeem-url-input" class="redeem-name-input-label">Примечание</label>-->
<!--                    <input type="text" id="redeem-url-input"-->
<!--                           class="calc-text-input redeem-input redeem-note-input redeem-note" placeholder="">-->
<!--                    <div class="input-notice"></div>-->
<!--                </div>-->
<!--            </div>-->
<!--        </div>-->
<!--    </div>-->
<!--    <div class="redeem-line2 redeem-params-margin">-->
<!--    </div>-->
<!---->
<!--</div>-->
<!--<div class="redeem-buttons" style="display: flex; width: 97%;">-->
<!--    <div class="add-container redeem-add" style="width: 180px;">-->
<!--        <div class="add-button">-->
<!--            <span class="plus">+</span>-->
<!--            <span class="add">Добавить товар</span>-->
<!--        </div>-->
<!--    </div>-->
<!--    <button class="calc-type-button tnved-calc-button submit-redeem-data submit-excel-file"-->
<!--            style="font-size: 18px; margin-left: 0;"><span class="dots-redeem">···</span>Прикрепить Excel-файл-->
<!--    </button>-->
<!--    <div class="selected-file-name">-->
<!--        <div class="del-file">-->
<!--            <div class="close-cross custom-span-tnved"-->
<!--                 style="left: unset; margin-left: unset; margin-top: 2px; display: block;">-->
<!--                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"-->
<!--                     width="29.5558703725" height="29.5558703725"-->
<!--                     viewBox="154.70573108694998 97.92453478204999 29.5558703725 29.5558703725" xml:space="preserve">-->
<!--                    <g transform="matrix(1 0 0 1 169.4836662732 112.7024699683)" id="k3KNRoqNc6poTgMk0-mS0">-->
<!--                        <g style="">-->
<!--                            <g transform="matrix(0.5361434886 0 0 0.5361434886 0 0)" id="TtVKn0Vpwc73qYTht60LM">-->
<!--                                <path class="cross-border"-->
<!--                                      style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                                      transform=" translate(0, 0)"-->
<!--                                      d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"-->
<!--                                      stroke-linecap="round"/>-->
<!--                            </g>-->
<!--                            <g transform="matrix(0.4501312202 0 0 0.4501312202 0 0)" id="UNUcbtI9Ts4pakmCW0QUX">-->
<!--                                <path class="cross-back"-->
<!--                                      style="stroke: rgb(179,25,24); stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                                      transform=" translate(0, 0)"-->
<!--                                      d="M 0 -27.5634 C 15.21499 -27.5634 27.5634 -15.215000000000002 27.5634 0 C 27.5634 15.21499 15.215000000000002 27.5634 0 27.5634 C -15.21499 27.5634 -27.5634 15.215000000000002 -27.5634 0 C -27.5634 -15.21499 -15.215000000000002 -27.5634 0 -27.5634 z"-->
<!--                                      stroke-linecap="round"/>-->
<!--                            </g>-->
<!--                            <g transform="matrix(0.4487730328 0 0 0.4487730328 0 0)" id="_16PNKPtF4U3-sJUah521">-->
<!--                                <path class="cross-path"-->
<!--                                      style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill-rule: nonzero; opacity: 1;"-->
<!--                                      transform=" translate(0, 0)"-->
<!--                                      d="M 14.14294 -14.1405 C 12.58094 -15.702499999999999 10.045939999999998 -15.702499999999999 8.484939999999998 -14.1405 L -0.00006000000000128125 -5.656499999999999 L -8.484060000000001 -14.1395 C -10.04606 -15.7015 -12.580060000000001 -15.7015 -14.14206 -14.1395 C -15.70406 -12.5775 -15.70406 -10.0435 -14.14206 -8.4835 L -5.657060000000001 0.0005000000000006111 L -14.14206 8.4845 C -15.70406 10.0465 -15.70406 12.5805 -14.14206 14.1405 C -12.580060000000001 15.702499999999999 -10.04606 15.702499999999999 -8.48406 14.1405 L -0.000059999999999504894 5.657499999999999 L 8.48394 14.1405 C 10.04594 15.702499999999999 12.57994 15.702499999999999 14.141940000000002 14.1405 C 15.703940000000001 12.5785 15.703940000000001 10.043499999999998 14.141940000000002 8.4845 L 5.656940000000002 0.0005000000000006111 L 14.141940000000002 -8.4835 C 15.703940000000001 -10.0465 15.703940000000001 -12.5795 14.142940000000001 -14.1405 z"-->
<!--                                      stroke-linecap="round"/>-->
<!--                            </g>-->
<!--                        </g>-->
<!--                    </g>-->
<!--                </svg>-->
<!--            </div>-->
<!--        </div>-->
<!--    </div>-->
<!--</div>-->
