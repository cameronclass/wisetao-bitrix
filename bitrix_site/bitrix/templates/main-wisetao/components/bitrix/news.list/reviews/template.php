...
<div class="reviews">
    <ul class="reviews__menu">
        <?foreach ($arTopics as $arTopic): ?>
            <li data-aos="fade-up" class="reviews__menu_list"><a class="reviews__menu_item" href="/<?=$_GET['direct-china']?>/<?=$_GET['data-in-menu']?>/reviews/?tag=<?=$arTopic['VALUE']?>"><?=$arTopic['VALUE']?></a></li>
        <?endforeach;?>
    </ul>
...
<!--                    <h2 class="group-title" style="margin-bottom: 0; margin-top: 30px; text-transform: uppercase; font-size: 25px;">--><?php //= $arItem["PROPERTIES"]["TOPICS"]["VALUE"]?><!--</h2>-->
<!--                --><?//endif;?>
                <?
                ...
                <? if ((isset($_REQUEST['tag']) && $this_tag || !isset($_REQUEST['tag']) || $_REQUEST['tag'] == 'Все') && $news_count < $_SESSION['n_count_requested']): ?>
                    <div itemscope itemtype="https://schema.org/Review" data-aos="fade-up" class="reviews__item" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                        ...
                        <? else: ?>
                            ...
                            <span style="display: none;" class="hide-part-review"><?= mb_substr($text, 304, mb_strlen($text) - 304); ?></span>
                            <?$button_opacity = ''?>
                        <? endif; ?>
                        <div class="reviews__item_block">
                            <div class="reviews__item_category">
                                <div class="reviews__item_category_tag <?=$arItem["PROPERTIES"]["MAIN_TOPIC"]["VALUE_XML_ID"];?>-tag"><?=$arItem["PROPERTIES"]["MAIN_TOPIC"]["VALUE"];?></div>
                                <div class="reviews__item_category_service">Услуга: <?=$arItem["PROPERTIES"]["SERVICES"]["VALUE"];?></div>
                            </div>
                            <button class="reviews__item_more <?=$button_opacity?>">
                                <span>Развернуть</span>
                                <img class="reviews__item_more_icon" src="<?=SITE_TEMPLATE_PATH?>/assets/images/icons/arrow-down.svg" alt="">
                            </button>
...
<div class="blog-page__articles_more" data-aos="fade-up">
    <button type="button" class="blog-page__articles_more_btn">
        ...
        ?>
        <a class="not-clickable" href="/<?=$_GET['direct-china']?>/<?=$_GET['data-in-menu']?>/reviews/?more=<?=$str_tag?>" style="display: flex;">
