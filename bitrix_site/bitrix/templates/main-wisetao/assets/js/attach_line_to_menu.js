...
function setPositionCardsLine(cardsLine, menuItemRect, cardsClass) {
    ...
    cardsLine.style.top = `${menuItemRect.top + menuItemRect.height / 2 - cardsLine.getBoundingClientRect().height / 2}px`;
    cardsLine.style.left = `${menuItemRect.left + menuItemRect.width}px`;
}

function hideLineCardsForMenuItem(menuItem) {
    let cardsClass = menuItem.classList[menuItem.classList.length - 1].split('__menu_item');
    const cardsLine = document.querySelector(`.${cardsClass[0]}__cards`);
...
function updateFixCardsPosition() {
    ...
    if (fixCards) {
        let menuItemClass = !fixCards.classList.contains('menu-item__cards_active')
            ? fixCards.classList[fixCards.classList.length - 1].split('__fix_cards_active')
            : fixCards.classList[fixCards.classList.length - 2].split('__fix_cards_active');
        const menuItem = document.querySelector(`.${menuItemClass[0]}__menu_item`);
        const menuItemRect = menuItem.getBoundingClientRect();
        fixCards.style.top = `${menuItemRect.top + menuItemRect.height / 2 - fixCards.getBoundingClientRect().height / 2}px`;
