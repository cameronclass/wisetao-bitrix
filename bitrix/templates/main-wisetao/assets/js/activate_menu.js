document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('.page-menu__menu_drop_link').forEach((item) => {
        if (item.nextElementSibling && item.nextElementSibling.classList.contains('page-menu__inside')) {
            if (item.getAttribute('href') !== '/from-china/logistic/' && item.getAttribute('href') !== '/in-china/logistic/') {
                item.setAttribute('href', updateURL(item.getAttribute('href')));
            }
            item.nextElementSibling.querySelector('.page-menu__inside_block').style.marginTop = '0';
            item.nextElementSibling.querySelector('.page-menu__inside_block').style.display = 'block';
...
// Функция для добавления обработчиков событий
    ...
    const addEventHandlers = () => {
        ...
        const links = document.querySelectorAll('.page-menu__menu_link');
        links.forEach(link => {
            if ((link.getAttribute('href') === '/hash-left-about/' || link.getAttribute('href') === '/hash-right-about/') && link.textContent !== 'О нас') {
...
// Функция для удаления обработчиков событий
    const removeEventHandlers = () => {
        let pageMenuActive = document.querySelector('.page-menu')?.classList.contains('active');
...
//     });
    ...
    function applyMenuStyles() {
        document.querySelectorAll('.page-menu__menu_drop_link').forEach((item) => {
            ...
            if (item.nextElementSibling && item.nextElementSibling.classList.contains('page-menu__inside')) {
                item.nextElementSibling.querySelector('.page-menu__inside_block').style.marginTop = '10px';
...
// Обработчик события mouseenter для элемента с классом .hover-for-menu
    document.querySelector('.hover-for-menu')?.addEventListener('mouseenter', applyMenuStyles);

// Обработчик события click для кнопки с классом .mobile-menu-open
    document.querySelector('.mobile-menu-open')?.addEventListener('click', applyMenuStyles);

    if (document.querySelector('.page-menu') && !document.querySelector('.page-menu').classList.contains('hovered')) {
        addLinks();
    }
...
//     document.querySelector('.hover-for-menu').addEventListener('mouseleave', removeEventHandlers);
    document.querySelector('.page-menu')?.addEventListener('mouseleave', (event) => {
    ...
    if (window.location.pathname.includes('data-in-level-')) {
        document.querySelectorAll('.main-menu-item__card').forEach((item) => {
            if (item.dataset.drop === window.location.pathname.split('data-in-level-')[1].split('/')[0]) {
                item.click();
            }
        });
    }
...
//     }
//
//     // Универсальный обработчик событий для кликов и наведений
...
//     };
//
//     // Поддержка и для мобильных устройств, и для десктопа
//     document.querySelector('.hover-for-menu')?.addEventListener('mouseenter', handleMenuInteraction);
//     document.querySelector('.hover-for-menu')?.addEventListener('click', handleMenuInteraction);
//
//     // document.querySelector('.mobile-menu-open')?.addEventListener('click', applyMenuStyles);
//     let mobileMenuOpenButtons = document.querySelectorAll('.mobile-menu-open');
//
//     if (mobileMenuOpenButtons.length > 0) {
//         mobileMenuOpenButtons.forEach(button => {
//             button.addEventListener('click', applyMenuStyles);
//         });
//     }
//
//
...
function addLinks() {
    ...
    links.forEach(link => {
        ...
        if (link.getAttribute('href').includes('hash-left') || link.getAttribute('href').includes('from-china/logistic')) {
            clone.textContent = 'ИЗ КИТАЯ';
            clone.setAttribute('href', '/hash-left-about/');
            clone.dataset.inMenu = 'about';
        } else if (link.getAttribute('href').includes('hash-right') || link.getAttribute('href').includes('in-china/logistic')) {
...
const updateURL = (url) => {
    ...
    if (hashPrefix) {
        ...
        pathSegments.forEach((segment, index) => {
            if ((segment === 'from-china' || segment === 'in-china') && index + 1 < pathSegments.length && !prefixAdded) {
...
function removeEllipsis() {
    ...
    if (pageMenu) {
        ...
        if (menuItemActive) {
            let pageMenuMenuDropActive =  menuItemActive.querySelector('.page-menu__menu_drop.active');
            if (pageMenuMenuDropActive) {
                let pageMenuMenuDropLinkActive = pageMenuMenuDropActive.querySelector('.page-menu__menu_drop_link.active');
                if (pageMenuMenuDropLinkActive &&
                    pageMenuMenuDropLinkActive.nextElementSibling &&
                    pageMenuMenuDropLinkActive.nextElementSibling.classList.contains('page-menu__inside')) {
                    const ellipsis = pageMenuMenuDropActive.querySelector('.page-menu__menu_drop_ellipsis');
                    if (ellipsis) {
                        pageMenuMenuDropActive.removeChild(ellipsis);
                        pageMenuMenuDropActive.style.marginTop = '8px';
                        pageMenuMenuDropLinkActive.classList.remove('page-menu__menu_drop_link-to-fit');
                        pageMenuMenuDropLinkActive.nextElementSibling.querySelector('.page-menu__inside_block').style.marginBottom = '10px';
                        pageMenuMenuDropLinkActive.nextElementSibling.querySelector('.page-menu__inside_block').style.paddingLeft = '20px';
                    }
                }
                const ellipsis = pageMenuMenuDropActive.parentElement.querySelector('.page-menu__menu_drop_ellipsis');
                if (ellipsis) {
                    pageMenuMenuDropActive.parentElement.removeChild(ellipsis);
                    pageMenuMenuDropActive.previousElementSibling.classList.remove('page-menu__menu_link-to-fit');
                }
            }
        }
    }
            ...
            if (menuItemActive.offsetWidth > 600) {
                ...
                if (pageMenuMenuDropActive) {
                    ...
                    if (pageMenuMenuDropLinkActive &&
                        pageMenuMenuDropLinkActive.nextElementSibling &&
                        pageMenuMenuDropLinkActive.nextElementSibling.classList.contains('page-menu__inside')) {
                        if (!pageMenuMenuDropLinkActive.parentElement.querySelector('.page-menu__menu_drop_ellipsis')) {
                            const ellipsis = document.createElement('span');
                            ellipsis.classList.add('page-menu__menu_drop_ellipsis');
                            ellipsis.innerText = '···';
                            ellipsis.addEventListener('click', (event) => {
                                showLink(event.currentTarget, 'page-menu__menu_drop_link-to-fit');
                            });
                            pageMenuMenuDropActive.insertBefore(ellipsis, pageMenuMenuDropLinkActive);
                            ...
                            pageMenuMenuDropLinkActive.addEventListener('mouseleave', () => {
                                if (pageMenuMenuDropLinkActive.previousElementSibling.classList.contains('page-menu__menu_drop_ellipsis')) {
                                    hideLink(pageMenuMenuDropActive, pageMenuMenuDropLinkActive);
                                }
                            });
                            pageMenuMenuDropLinkActive.nextElementSibling.querySelector('.page-menu__inside_block').style.marginBottom = '5px';
                            pageMenuMenuDropLinkActive.nextElementSibling.querySelector('.page-menu__inside_block').style.paddingLeft = '0';
                        }
                    }
                    if (menuItemActive.offsetWidth > 600) {
                        if (!pageMenuMenuDropActive.parentElement.querySelector('.page-menu__menu_drop_ellipsis')) {
                            const ellipsis = document.createElement('span');
                            ellipsis.classList.add('page-menu__menu_drop_ellipsis');
                            ellipsis.innerText = '···';
                            ellipsis.addEventListener('click', (event) => {
                            ...
                            pageMenuMenuDropActive.parentElement.insertBefore(ellipsis, pageMenuMenuDropActive.previousElementSibling);
                            pageMenuMenuDropActive.previousElementSibling.classList.add('page-menu__menu_link-to-fit');
                            pageMenuMenuDropActive.previousElementSibling.addEventListener('mouseleave', () => {
                                if (pageMenuMenuDropActive.previousElementSibling.previousElementSibling.classList.contains('page-menu__menu_drop_ellipsis')) {
                                    hideLinkTwo(pageMenuMenuDropActive, pageMenuMenuDropActive.previousElementSibling);
                                }
                            });
                        }
                    }
...
function activateBreadCrumbs() {
    ...
    let menuLinksActive = document.querySelectorAll('.page-menu__menu_link.active');
    let menuDropLinksActive = document.querySelectorAll('.page-menu__menu_drop_link.active');

    if (menuItemActive) {
        menuItemActive.setAttribute('itemscope' , '');
        menuItemActive.setAttribute('itemtype' , "https://schema.org/BreadcrumbList");
    }
...
function addMicrodata(item, index) {
    ...
    linkItem.setAttribute('content', new URL(item.getAttribute('href'), window.location.href).href);
