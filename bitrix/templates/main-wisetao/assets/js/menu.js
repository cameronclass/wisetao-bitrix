...
function initMenu() {
    /* Внутреннее меню */
    let dataInButton = document.querySelectorAll(".page-menu__menu_group_items.active [data-in-menu]");
    let dataInLevelButton = document.querySelectorAll(".page-menu__menu_group_items.active [data-in-level]");

    if (dataInButton)
        dataInButton.forEach(function (button) {
            button.addEventListener("click", function () {
                const dataInContent = document.querySelectorAll(".page-menu__menu_group_items.active [data-in-content]");
