...
function calcLineTags() {
    ...
    if (inputSearch.dataset.ajax_from_blog === '1') {
        ...
        let blogPage__menu_listWidth = blogPage__menu_list.getBoundingClientRect().width;
        let children = blogPage__menu_more.childNodes;
        let j = 2;
        for (let i = children.length - 1; i >= 2; i--) {
            blogPage__menu_more.insertBefore(children[children.length - 1], blogPage__menu_more.childNodes[j]);
            j++;
        }

        while (blogPageWidth > blogPage__menu_listWidth && blogPage__menu_more.childElementCount > 1) {
            ...
            if (blogPageWidth < blogPage__menu_listRect.width) {
                blogPage__menu_more.insertBefore(blogPage__menu_more_link, blogPage__menu_more.lastChild);
        ...
        if (blogPage__menu_more.childElementCount === 1) {
            document.querySelector('.blog-page__menu').removeChild(document.querySelector('.blog-page__menu_more'));
        } else {
