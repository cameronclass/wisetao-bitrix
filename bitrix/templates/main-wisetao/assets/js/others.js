...
document.addEventListener("DOMContentLoaded", function () {
  ...
  reviewDropdownToggle = document.querySelector(
  ...
  if (questionBlockForm) {
    questionInputsNotice = questionBlockForm.querySelectorAll(
      ".input-form__notice"
    );
    questionInputs = questionBlockForm.querySelectorAll(
      ".question-block__form_input"
    );
  }

  orderServiceBlockForm = document.querySelector(".ask-panel__form");
  if (orderServiceBlockForm) {
    orderServiceInputsNotice = orderServiceBlockForm.querySelectorAll(
      ".input-form__notice"
    );
    orderServiceInputs = orderServiceBlockForm.querySelectorAll(
      ".order-service__form_input"
    );
    datasetAskInc =
      orderServiceBlockForm.parentElement.parentElement.querySelector(
        ".js-ask-close"
      );
    orderServiceButton =
      orderServiceBlockForm.parentElement.parentElement.querySelector(
...
function initializeAskPanelAjaxMode() {
  ...
  orderServiceButton = askPanelForm?.parentElement.parentElement.querySelector(
    'button[name="web_form_submit"]'
  );
  ...
  async function showCompleteFormViewSuccess(event) {
    if (askPanelForm && askPanelForm.checkValidity()) {
      event.preventDefault();
      let clientName = document.querySelector(
        ".ask-panel__form .client-name"
      )?.value;
      let clientPhone = document.querySelector(
        ".ask-panel__form .phone"
      )?.value;
      let clientEmail = document.querySelector(
        ".ask-panel__form .email"
      )?.value;
      let serviceName = document.querySelector(
        ".ask-panel__submit .service-name"
      )?.textContent;
      let serviceQuestion = document.querySelector(
        ".ask-panel__submit .service-question"
      )?.value;
  ...
  if (
    ...
    orderServiceButton?.addEventListener(
      "click",
      listenerInitializeAskPanelFormSubmit
    );
  }
...
function initializeQuestionFormAjaxMode() {
  ...
  async function showCompleteFormViewQuestionBefore(event) {
    if (questionBlockForm && questionBlockForm.checkValidity()) {
      event.preventDefault();
      let clientName = document.querySelector(
        ".question-block__submit .client-name"
      )?.value;
      let clientPhone = document.querySelector(
        ".question-block__submit .phone"
      )?.value;
      let clientEmail = document.querySelector(
        ".question-block__submit .email"
      )?.value;
      if (clientPhone && (await validateNumber(clientPhone))) {
...
function initializeValidateQuestionForm() {
  ...
  function initializeInputs(input, key) {
    ...
    if (
      typeof listenerInitializeQuestionInputsInput[key] === "undefined" ||
      ajax_question_inc <
        questionBlockForm.parentElement.dataset.ajax_question_inc
    ) {
    ...
    if (
      typeof listenerInitializeQuestionInputsBlur[key] === "undefined" ||
      ajax_question_inc <
        questionBlockForm.parentElement.dataset.ajax_question_inc
    ) {
...
function initializeValidateServiceOrderForm() {
  if (orderServiceInputsNotice) {
    orderServiceBlockForm = document.querySelector(".ask-panel__form");
    orderServiceInputs = document.querySelectorAll(
      ".order-service__form_input"
    );
    datasetAskInc =
      orderServiceBlockForm.parentElement.parentElement.querySelector(
  ...
  function initializeInputs(input, key) {
    ...
    if (
      typeof listenerInitializeOrderServiceInputsInput[key] === "undefined" ||
    ...
    input.addEventListener(
      "input",
      listenerInitializeOrderServiceInputsInput[key]
    );

    if (
      typeof listenerInitializeOrderServiceInputsBlur[key] === "undefined" ||
      ajax_ask_inc < datasetAskInc.dataset.ajax_ask_inc
    ) {
    ...
    input.addEventListener(
      "blur",
      listenerInitializeOrderServiceInputsBlur[key]
    );
  }
...
function hideNotice(input) {
  ...
  let noticeInvalidNumber = input.parentElement.querySelector(
    ".input-form__notice-valid-number"
  );
...
async function sendWisetaoFormData(
  clientName,
  clientPhone,
  clientEmail,
  isClientExist,
  ...
  let query = {
    action:
      "telegram:document.api.WisetaoFormsController.send_wisetao_forms_data_to_deal_bitrix24",
  };
