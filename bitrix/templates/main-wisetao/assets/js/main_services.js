jQuery(document).ready(function($) {
	...
	const swiperMainService = new Swiper('.block-mainServices .swiper', {
		...
		pagination: {
			...
			renderCustom: (swiper, current, total) =>
				`<span class="current-number">${formatNumber(current)}</span> / <span class="total-number">${formatNumber(total)}</span>`,
		},
		breakpoints: {
			1: { autoHeight: true },
	...
	const scrollableBlocks = document.querySelectorAll('.block-mainServices__body .mainService-item__bg');
	const minWidth = 992;
		...
		scrollableBlocks.forEach(element => {
			if (servicesBlock && element) {
				...
				const shiftSlider = () => {
					...
					const shift = Math.max(Math.min(Math.round(top), window.innerHeight - hws), -scrollHeight + hws);
